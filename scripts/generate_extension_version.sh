...
if [ -f $gitIndexDir ]; then  
    ...
    # In case not in a git repo, for example in tarball of release, or there is no git index file
    # we will use the version from the control file
    # and set the GIT_VERSION to unknown
    # and GIT_SHA to unknown
    # and BUILD_VER to empty
    # and EXTENSION_VERSION_STR to the version in the control file
    ...
    echo "#define GIT_VERSION \"unknown sha:unknown\"" >> $outputFile
    exit 0
fi   
